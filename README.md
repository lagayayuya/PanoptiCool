...
- **Le mur sémantique.** Moins de X % du volume d'un export est auto-descriptif hors-ligne : les liens de vidéos sont opaques. Le profil se reconstruit depuis les **recherches, commentaires et comptes suivis**, pas depuis les liens. Mais il s'agit dans le même temps d'un argument fort : En ne pouvant interpréter qu'un faible pourcentage de l'export (généralement entre 0 et 5%), PanoptiCool révèle déja un ensemble d'informations très riche, bien que cette première version n'exploite pas tout le potentiel des export TikTok
