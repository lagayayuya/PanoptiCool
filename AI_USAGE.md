...
- **Jugement humain :** Limite posée par Yul : brancher sans modifier le moteur, corriger seulement si un vrai fichier révèle un problème. Le panneau de debug ré-estime de son côté ce que le moteur a exploité plutôt que d'instrumenter le moteur — prématuré pour un outil temporaire — à condition d'afficher que c'est une approximation, pour qu'elle ne passe pas pour un fait. Trafic réseau inspecté pendant les essais : rien ne sort de l'appareil. L'agent explicite ce qu'il ne peut pas vérifier lui-même — le passage avec un vrai fichier reste au mainteneur, sans qu'aucun contenu n'entre dans la conversation.
